...
def gradient_descent_runner(
    ...
    for i in range(num_iterations):
        ...
        if i % 10 == 0:
            plotdata.append(
                np.array([b, m, compute_error(b, m, points)])
            )
    return [b, m], plotdata
...
def get_line(m_slope=1, c_intercept=0, initial_b=0, initial_m=0, learning_rate=0.001, num_pts=150):
    num_pts = min(200,num_pts)
    fig = figure()
    plotdata = []
    x = np.linspace(1, 50, num_pts)
    points = np.array([x, Polynomial_Fit.get_noisy_func(x, 1, lambda x: m_slope*x+c_intercept, num_pts=num_pts)]).T
...
#        lambda iterations: update_plot(iterations, fig, plotdata, [initial_b, initial_m, learning_rate,points], (fig1[0], fig2[0], fig4[0])),
        lambda iterations: update_plot(iterations, fig, plotdata, [initial_b, initial_m, learning_rate,points], (fig4[0])),
